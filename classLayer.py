...
class Layer:
    def __init__(self, weights, bias, type_input = False):
        ...
        self.n_inputs = self.n_neurons if type_input else weights.shape[1]
    ...
    def evaluate(self, inputs):
        ...
        if self.type_input: return np.array([self.neurons[i].forward(inputs[i]) for i in range(self.n_neurons)])
